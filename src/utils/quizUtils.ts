...
export const getQuestionsBySubject = (subjectId: Subject, count: number): Question[] => {
  switch (subjectId) {
...
export const getQuestionsForQuiz = (quiz: QuizConfig): Question[] => {
  return getQuestionsBySubject(quiz.subjectId as Subject, quiz.questionCount);
};
...
export const calculateScore = (questions: Question[], answers: Record<string, number | null>): number => {
  let correctCount = 0;
...
export const analyzeTopics = (questions: Question[], answers: Record<string, number | null>) => {
  ...
  return Object.entries(topicsAnalysis).map(([topic, data]) => ({
    topic,
